export const DEFAULT_ACTION_DIR = "actions";
export const MAP_FILE_NAME = "sfa-router.map";
export const METADATA = "__@sfajs/router_metadata__";
export const STARTUP_ROUTER_CONFIG = "__@sfajs/router_startupRouterConfig__";
export const REQUEST_HTTP_CONTEXT = "__@sfajs/router_requestHttpContext__";
export const HTTP_CONTEXT_STARTUP = "__@sfajs/router_httpContextStartup__";
export const REQUEST_CACHE_PARAMS = "__@sfajs/router_ctxCacheParams__";
export const CTX_CACHE_METADATA = "__@sfajs/router_ctxCacheMetadata__";
